...
dpkg -l | grep -Ei 'buddy-panel-service|busybox|PROTECTED|coreutils|corosync|crmsh|crossbar|efpinstall|evertz|logrotate|magnum|mdl|ntp|pacemaker|snmp|systemd|vueweb' | tee $dir/$state/$host/dpkg.txt | grep -Ei '${service_long}'
echo
ENDSSH
...
sshpass -p $password scp -r $user@$host:/home/PROTECTED/$dir $drive_prefix
echo
...
for host in "${hosts[@]}"; do 
sshpass -p $password scp $drive_prefix/magnum-reflex-3.0.8_bionic.efp.md5 $user@$host:/home/PROTECTED
sshpass -p $password scp $drive_prefix/magnum-reflex-3.0.8_bionic.efp     $user@$host:/home/PROTECTED
echo
...
dpkg -l | grep -Ei 'buddy-panel-service|busybox|PROTECTED|coreutils|corosync|crmsh|crossbar|efpinstall|evertz|logrotate|magnum|mdl|ntp|pacemaker|snmp|systemd|vueweb' | tee $dir/$state/$host/dpkg.txt | grep -E 'rootfs|${service_long}'
echo
ENDSSH
